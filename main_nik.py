			...
			for j in range(0, len(classes)):
				if(j >= 1):
					if(classes[i].get_meetingTime() == classes[j].get_meetingTime() and classes[i].get_idno()!= classes[j].get_idno()):
						if(classes[i].get_room() == classes[j].get_room()): self._numbOfConflicts = self._numbOfConflicts + 1
						if(classes[i].get_instructor() == classes[j].get_instructor()): self._numbOfConflicts = self._numbOfConflicts + 1
...
class GeneticAlgo:
    ...
    def _crossover_schedule(self, schedule1, schedule2):
        crossoverSchedule = Schedule.initalize()
...
class DisplayMgr:
    ...
    def print_course(self):
        ...
        for i in range(0, len(courses)):
            instructors = courses[i].get_courses()
            tmepStr = ""
    ...
    def print_room(self):
        ...
        for i in range(0 ,len(rooms)):
            avilableRoomsTable.add_row([str(rooms[i].get_number()), str(rooms[i].get_seatingCapacity())])
        print(availableRoomsTable)
    def print_meeting_times(self):
        avilableMeetingTimeTable = prettytable.PrettyTable(['id', 'Meeting Time'])
        meetingTimes = data.get_meetingTimes()
        for i in range(0, len(meetingTimes)):
            avilableMeeitngTimeTable.add_row([meetingTimes[i].get_idno(), meetingTimes[i].get_time()])
        print(availableMeetingTimeTable)
    def print_generation(self, population):
        table1 = prettytable.PrettyTable(['schedule #', 'fitness', '# of conflicts', 'classes [dept,class,room,instructors]'])
    ...
    def print_schedule_as_table(self, schedule):
        ...
        table = prettytable.PrettyTable(['Class #', 'Dept', 'Course (number, max # of students)', 'Room (Capacity)', 'Instructors'])
        for i in range(0, len(classes)):
            table.add_row([str(i), classes[i].get_dept().get_name(), classes[i].get_course().get_name() + " (" + 
                           ...
                           classes[i].get_instructor().get_name() + " (" + str(classes[i].get_instructor.get_idno()) + ")",
                           classes[i].get_meetingTime().get_time() + " (" + str(classes[i].get_meetingTime().get_idno()) + ")"])
        print(table)

data = Data()
displayMgr = DisplayMgr()
displayMgr.print_avilable_data()
generationNumber = 0
